window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let modelId = readStoredId("afterglow-modelId");
		let modelTexturesId = readStoredId("afterglow-modelTexturesId");
		let config = {};
		let messageArray = [];
		function getModelId() {
		...
		function setModelId(newModelId) {
			modelId = newModelId;
			localStorage.setItem("afterglow-modelId", newModelId.toString());
		}
		...
		function setModelTexturesId(newModelTexturesId) {
			...
			localStorage.setItem("afterglow-modelTexturesId", newModelTexturesId.toString());
		}
		function resetModelState() {
			modelId = 0;
			modelTexturesId = 0;
			localStorage.setItem("afterglow-modelId", "0");
			localStorage.setItem("afterglow-modelTexturesId", "0");
		}
		function getConfig() {
			return config;
		}
		...
		function showMessage(model, text, timeout, priority) {
			if (!text) return;
			const storedPriority = parseInt(sessionStorage.getItem("afterglow-waifu-text"), 10);
			...
			sessionStorage.setItem("afterglow-waifu-text", priority);
			const tips = document.getElementById("waifu-tips-afterglow");
			if (tips) {
				tips.innerHTML = text.text || "";
				tips.classList.add("waifu-tips-active");
			}
			messageTimer = setTimeout(() => {
				sessionStorage.removeItem("afterglow-waifu-text");
		...
		*/
		...
		var Model = class {
			constructor() {
				...
				this.app = new PIXI.Application({
					view: document.getElementById("live2d-afterglow"),
					autoStart: true,
					width: 800,
					height: 800,
					backgroundAlpha: 0
		...
		const tools = {
			...
			"quit": {
				icon: fa_xmark,
				callback: () => {
					localStorage.setItem("afterglow-waifu-display", Date.now());
					const waifu = document.getElementById("waifu-afterglow");
					if (waifu) waifu.style.bottom = "-500px";
					setTimeout(() => {
						const toggle = document.getElementById("waifu-toggle-afterglow");
    ...
    <div id="waifu-afterglow">
      <canvas id="live2d-afterglow" width="800" height="800"></canvas>
      <div id="waifu-tips-afterglow"></div>
      <div id="waifu-tool-afterglow"></div>
    </div>
    <div id="model-selection-panel-afterglow" class="waifu-panel waifu-panel-afterglow" style="display: none;"></div>
    <div id="texture-selection-panel-afterglow" class="waifu-panel waifu-panel-afterglow" style="display: none;"></div>`);
			const model = new Model();
			localStorage.removeItem("afterglow-waifu-display");
			sessionStorage.removeItem("afterglow-waifu-text");
			const waifu = document.getElementById("waifu-afterglow");
			const toolBar = document.getElementById("waifu-tool-afterglow");
			const modelPanel = document.getElementById("model-selection-panel-afterglow");
			const texturePanel = document.getElementById("texture-selection-panel-afterglow");
			...
			for (const tool of getConfig().tools) {
				...
				toolBar.insertAdjacentHTML("beforeend", `<span id="waifu-tool-afterglow-${tool}" title="${TOOL_TITLES[tool] || tool}">${decodeURIComponent(icon).replace("data:image/svg+xml,", "")}</span>`);
				document.getElementById(`waifu-tool-afterglow-${tool}`).addEventListener("click", callback);
			}
			...
			hooks.on(document, "click", (event) => {
				if (event.target.closest("#model-selection-panel-afterglow") || event.target.closest("#texture-selection-panel-afterglow") || event.target.closest("#waifu-tool-afterglow") || event.target.closest("#waifu-toggle-afterglow")) return;
				closePanels();
			});
		...
		function enableDrag(widgetEl) {
			...
			widgetEl.addEventListener("pointerdown", (event) => {
				if (event.target.closest("#waifu-tool-afterglow") || event.target.closest(".waifu-panel-afterglow") || event.target.closest("#waifu-toggle-afterglow")) return;
				drag.active = true;
				drag.moved = false;
			...
			const endDrag = (event) => {
				...
				if (drag.moved) {
					const rect = widgetEl.getBoundingClientRect();
					try {
						localStorage.setItem("afterglow-waifu-pos", JSON.stringify({
							left: rect.left,
							top: rect.top
						}));
					} catch (error) {}
		...
		function restorePosition(widgetEl) {
			try {
				const pos = JSON.parse(localStorage.getItem("afterglow-waifu-pos"));
		...
		function registerEventListener(model, drag, hooks) {
			...
			hooks.on(window, "mouseover", (event) => {
				if (event.target.closest("#live2d-afterglow")) {
		...
		*/
		async function initWidget(config) {
			...
			setConfig(config);
			document.getElementById("waifu-toggle-afterglow")?.remove();
			document.getElementById("waifu-afterglow")?.remove();
			document.body.insertAdjacentHTML("beforeend", `<div id="waifu-toggle-afterglow"><span>Live2D</span></div>`);
			const toggle = document.getElementById("waifu-toggle-afterglow");
			let stopWidget = () => {};
			const toggleStop = () => {
				hooks.stop();
			...
			hooks.on(toggle, "click", async () => {
				...
				if (toggle.getAttribute("first-time")) {
					...
					localStorage.removeItem("afterglow-waifu-display");
					const waifuEl = document.getElementById("waifu-afterglow");
					if (waifuEl) {
			...
			if (localStorage.getItem("afterglow-waifu-display") && Date.now() - localStorage.getItem("afterglow-waifu-display") <= 864e5) {
...
#waifu-afterglow, #waifu-toggle-afterglow { z-index: 2147483646 !important; }
.waifu-panel { z-index: 2147483647 !important; }
#waifu-afterglow { left: auto; right: 20px; top: 20px; bottom: auto; }
`;
